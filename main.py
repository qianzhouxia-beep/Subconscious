...
def tarot_reading():
    ...
    if lang == 'zh':
        system_content = (
            ...
            "5. 结尾给出一句有启发性的建议或箴言（直接引用箴言本身，不要加「箴言：」这种前缀）\n\n"
            "格式要求：使用自然段落，每部分用 **加粗标题** 区分（如「**过去的能量**」）。"
            "箴言部分使用 **箴言** 作为加粗标题，后面直接是箴言文本，不再加冒号。"
            "语言温暖但不浮夸，"
        ...
        system_content = (
            ...
            "5. End with an inspiring suggestion or advice (present the advice directly, no prefix like 'Wisdom:')\n\n"
            "Format: Use natural paragraphs with **bold headers** (e.g. '**The Past**'). "
            "For the wisdom section, use '**Wisdom**' as the bold header, followed by the advice text directly without a colon. "
